...
for j=1:1:length(t);
    ...
    subplot(1,3,1);
    imagesc(y,x,pj,[-1 1]*1e-1);title('p');colormap(gray);axis equal tight;
    subplot(1,3,2);
    imagesc(y,x,uj,[-1 1]*1e-7);title('u');colormap(gray);axis equal tight;
    subplot(1,3,3);
    imagesc(y,x,vj,[-1 1]*1e-7);title('v');colormap(gray);axis equal tight;  
    drawnow;
    pause(.001);
end