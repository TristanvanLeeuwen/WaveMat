classdef opCurl < opSpot
%OpCurl 3-dimensional Curl operaror based on spectral
%differences
%
% use:
%   Curl = opCurl(N,L,method,store)
%
% input:
...
%   store  - whether to store or generate on-the-fly
   ...
   methods
      ...
      % opGrad Constructor.
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      function op = opCurl(N,L,method,store)
         % number of dimensions
         nd = length(N);
         
         op = op@opSpot('Curl',nd*prod(N),nd*prod(N));
         op.A           = cell(1,nd);
         op.x           = cell(1,nd);
         op.N           = N;
         op.L           = L;
   ...
   methods( Access = private )
       ...
       function y = opGrad_multiply(op,x,mode)
           LCS(:,:,1) = [0 0 0;0 0 1;0 -1 0];
           LCS(:,:,2) = [0 0 -1;0 0 0;1 0 0];
           LCS(:,:,3) = [0 1 0;-1 0 0;0 0 0];
           nd    = length(op.N);
           if mode == 1
